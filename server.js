...
async function getReferralChain(wallet) {
  ...
  for (let level = 1; level <= 3; level++) {
    // Ищем реферера напрямую по wallet в user_settings
    const r = await q(
      `SELECT referred_by FROM user_settings
       WHERE wallet = $1
...
async function getRecentIncomingTxs() {
  try {
    ...
    const url = `${CONFIG.AXIOME_REST}/cosmos/tx/v1beta1/txs?query=wasm.action%3D${actionEnc}%20AND%20wasm.to%3D${walletEnc}&limit=200`;
