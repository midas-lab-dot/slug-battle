...
app.post('/admin/confirm-payment', requireAdmin, async (req, res) => {
  const { battle_id, wallet, tx_hash, amount } = req.body;
  if (!battle_id || !wallet || !tx_hash) return res.status(400).json({ error: 'battle_id, wallet, tx_hash required' });

  // Проверяем что битва существует и участник зарегистрирован
  ...
  await confirmPayment(battle_id, wallet, tx_hash, amount || parseInt(b.stake) * 1e6);
