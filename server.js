...
async function getRecentIncomingTxs() {
  try {
    ...
    if (CONFIG.ESCROW_CONTRACT) {
      ...
      url = `${CONFIG.AXIOME_REST}/cosmos/tx/v1beta1/txs?events=wasm._contract_address%3D${escrowEnc}&limit=30`;
    } else {
      // Fallback режим: ищем CW20 SLUG переводы на PROJECT_WALLET
      // Правильный формат: query=wasm.action='transfer' AND wasm.to='wallet'
      const walletEnc = encodeURIComponent(`'${CONFIG.PROJECT_WALLET}'`);
      const actionEnc = encodeURIComponent(`'transfer'`);
      url = `${CONFIG.AXIOME_REST}/cosmos/tx/v1beta1/txs?query=wasm.action%3D${actionEnc}%20AND%20wasm.to%3D${walletEnc}&limit=30`;
    }
...
async function checkPayments() {
  ...
  for (const tx of await getRecentIncomingTxs()) {
    ...
    if (CONFIG.ESCROW_CONTRACT) {
      ...
      // Fallback режим: wallet из memo ИЛИ из wasm event
      const wallet = parsed.wallet || eventWallet;
      if (!wallet) continue;
      const pend = active.find(p => p.battle_id === parsed.battleId && p.wallet === wallet);
      if (!pend) continue;
      if (paidAmount !== null && paidAmount < parseInt(pend.amount)) continue;
      await confirmPayment(parsed.battleId, wallet, txHash, paidAmount);
    }
