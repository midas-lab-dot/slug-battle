...
async function getRecentIncomingTxs() {
  try {
    ...
    if (CONFIG.ESCROW_CONTRACT) {
      // Escrow режим: ищем wasm execute на escrow контракт
      // Событие: wasm._contract_address = ESCROW_CONTRACT
      const escrowEnc = encodeURIComponent(`'${CONFIG.ESCROW_CONTRACT}'`);
      url = `${CONFIG.AXIOME_REST}/cosmos/tx/v1beta1/txs?events=wasm._contract_address%3D${escrowEnc}&order_by=ORDER_DESC&limit=30`;
    } else {
      // Fallback режим: ищем переводы SLUG на PROJECT_WALLET
      const contractEnc = encodeURIComponent(`'${CONFIG.SLUG_CONTRACT}'`);
      const walletEnc   = encodeURIComponent(`'${CONFIG.PROJECT_WALLET}'`);
      url = `${CONFIG.AXIOME_REST}/cosmos/tx/v1beta1/txs?events=wasm._contract_address%3D${contractEnc}&events=wasm.recipient%3D${walletEnc}&order_by=ORDER_DESC&limit=30`;
    }
...
// Извлекаем wallet и сумму из wasm events транзакции
function extractPaymentInfo(tx) {
  try {
    for (const ev of tx.logs?.[0]?.events || []) {
      ...
      if (CONFIG.ESCROW_CONTRACT) {
        // Escrow режим: ищем событие join_battle
        // sender = кошелёк игрока, amount = ставка
        if (attrs.action === 'join_battle' || attrs.action === 'receive') {
          return {
            wallet: attrs.buyer || attrs.sender || attrs.player || null,
            amount: attrs.stake ? parseInt(attrs.stake) : null,
          };
        }
      } else {
        // Fallback: ищем transfer на PROJECT_WALLET
        if (attrs.recipient === CONFIG.PROJECT_WALLET && attrs.amount) {
          return { wallet: null, amount: parseInt(attrs.amount) };
        }
      }
    }
...
async function checkPayments() {
  ...
  for (const tx of await getRecentIncomingTxs()) {
    ...
    const { wallet: eventWallet, amount: paidAmount } = extractPaymentInfo(tx);

    if (CONFIG.ESCROW_CONTRACT) {
      // Escrow режим: wallet берём из события блокчейна
      // Ищем все pending платежи для этой битвы
      const battlePending = active.filter(p => p.battle_id === parsed.battleId);
      for (const pend of battlePending) {
        // wallet либо из события, либо ищем по battle_id
        const wallet = eventWallet || pend.wallet;
        if (paidAmount !== null && paidAmount < parseInt(pend.amount)) continue;
        await confirmPayment(parsed.battleId, wallet, txHash, paidAmount);
        break; // одна tx = один участник
      }
    } else {
      // Fallback режим: wallet из memo
      if (!parsed.wallet) continue;
