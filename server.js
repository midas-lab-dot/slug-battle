...
async function initDB() {
  ...
  // Миграция seen_txs — добавляем battle_id
  await q('ALTER TABLE seen_txs ADD COLUMN IF NOT EXISTS battle_id TEXT DEFAULT \'\'');
  await q('CREATE UNIQUE INDEX IF NOT EXISTS idx_seen_txs_battle ON seen_txs(tx_hash, battle_id)');
...
async function getUserLangById(tgUserId) {
...
const MSG = {
  stake_accepted: {
    ru: (battleId, paid, max) => `✅ Ставка принята! Битва #${battleId}. Оплатили: ${paid}/${max}`,
    en: (battleId, paid, max) => `✅ Stake accepted! Battle #${battleId}. Paid: ${paid}/${max}`,
  },
  battle_started: {
    ru: (battleId, bank, prize) => `🚀 Битва #${battleId} началась!\n\nБанк: ${bank.toLocaleString()} SLUG\nПриз: ${prize.toLocaleString()} SLUG\n\nЧей портфель вырастет больше — забирает всё!`,
    en: (battleId, bank, prize) => `🚀 Battle #${battleId} started!\n\nPool: ${bank.toLocaleString()} SLUG\nPrize: ${prize.toLocaleString()} SLUG\n\nWhose portfolio grows more — takes it all!`,
  },
  winner: {
    ru: (battleId, prize, txHash) => `🏆 <b>Ты победил!</b>\n\nБитва: <b>#${battleId}</b>\nПриз: <b>${prize.toLocaleString()} SLUG</b>\n\n💰 Токены уже на твоём кошельке!\nТранзакция: <code>${txHash}</code>`,
    en: (battleId, prize, txHash) => `🏆 <b>You won!</b>\n\nBattle: <b>#${battleId}</b>\nPrize: <b>${prize.toLocaleString()} SLUG</b>\n\n💰 Tokens sent to your wallet!\nTransaction: <code>${txHash}</code>`,
  },
  loser: {
    ru: (battleId, winner, growth) => `Битва #${battleId} завершена. Победитель: ${winner.slice(0,8)}... Твой результат: ${growth.toFixed(2)}%`,
    en: (battleId, winner, growth) => `Battle #${battleId} ended. Winner: ${winner.slice(0,8)}... Your result: ${growth.toFixed(2)}%`,
  },
  warn_5min: {
    ru: (battleId) => `⏰ <b>Битва #${battleId} заканчивается через 5 минут!</b>\n\nПроверь гонку — кто лидирует?`,
    en: (battleId) => `⏰ <b>Battle #${battleId} ends in 5 minutes!</b>\n\nCheck the race — who's leading?`,
  },
  watch_race: {
    ru: '🐌 Смотреть гонку',
    en: '🐌 Watch race',
  ...
  refund: {
    ru: (battleId, stake) => `↩️ <b>Битва #${battleId} отменена</b>\n\nНикто не присоединился за 2 часа.\nСтавка <b>${stake} SLUG</b> возвращена на твой кошелёк автоматически.`,
    en: (battleId, stake) => `↩️ <b>Battle #${battleId} cancelled</b>\n\nNobody joined within 2 hours.\nStake <b>${stake} SLUG</b> returned to your wallet automatically.`,
  },
  cancel_no_players: {
    ru: (battleId) => `❌ <b>Битва #${battleId} отменена</b>\n\nНикто не вошёл за 2 часа.`,
    en: (battleId) => `❌ <b>Battle #${battleId} cancelled</b>\n\nNobody joined within 2 hours.`,
  },
};
...
async function getReferralChain(wallet) {
  ...
  for (let level = 1; level <= 3; level++) {
    // Ищем реферера по wallet напрямую
    // Также ищем через tg_user_id из participants на случай если wallet добавлен позже
    const r = await q(
      `SELECT us.referred_by
       FROM user_settings us
       WHERE us.wallet = $1
         AND us.referred_by IS NOT NULL
       UNION
       SELECT us.referred_by
       FROM participants p
...
async function notifyWinner(battle, winner, prize) {
  ...
  const lang = await getUserLangById(p?.tg_user_id);
  const result = await autoPayout(battle.id, winner, prize);

  if (result.ok) {
    if (p?.tg_user_id) {
      await sendTg(p.tg_user_id, MSG.winner[lang](battle.id, prize, result.txHash));
    }
    const adminId = process.env.ADMIN_TG_ID;
    if (adminId) {
      await sendTg(adminId,
        `✅ <b>Battle #${battle.id} ended</b>\nWinner: <code>${winner}</code>\nPaid: ${prize.toLocaleString()} SLUG\nTx: <code>${result.txHash}</code>`
      );
    }
  } else {
    ...
    if (adminId) {
      await sendTg(adminId,
        `⚠️ <b>Auto payout failed!</b>\nBattle #${battle.id}\nWinner: <code>${winner}</code>\nPrize: ${prize.toLocaleString()} SLUG\nError: ${result.error}\n\nSign manually in Axiome Connect:`
      );
      await sendTg(adminId, `<code>${qr}</code>`);
    }
    if (p?.tg_user_id) {
      const pending = { ru: 'Выплата обрабатывается, скоро получишь токены.', en: 'Payout is being processed, you will receive tokens soon.' };
      await sendTg(p.tg_user_id,
        `🏆 <b>${lang==='en'?'You won!':'Ты победил!'}</b>\n\n${lang==='en'?'Battle':'Битва'}: <b>#${battle.id}</b>\n${lang==='en'?'Prize':'Приз'}: <b>${prize.toLocaleString()} SLUG</b>\n\n${pending[lang]}`
      );
    }
  }
}
...
async function notifyWinnerAuto(battle, winner, prize) {
  ...
  const lang = await getUserLangById(p.tg_user_id);
  await sendTg(p.tg_user_id, MSG.winner[lang](battle.id, prize, ''));
}
...
async function getRecentIncomingTxs() {
  try {
    ...
    // Берём 200 последних транзакций чтобы не пропустить
    const url = `${CONFIG.AXIOME_REST}/cosmos/tx/v1beta1/txs?query=wasm.action%3D${actionEnc}%20AND%20wasm.to%3D${walletEnc}&limit=200&pagination.limit=200`;
...
async function confirmPayment(battleId, wallet, txHash, paidAmount) {
  // Проверяем что эта транзакция не была уже засчитана для ЭТОЙ битвы
  const already = await q('SELECT 1 FROM seen_txs WHERE tx_hash=$1 AND battle_id=$2', [txHash, battleId]);
  if (already.rows.length) return;
  await q('INSERT INTO seen_txs (tx_hash, battle_id) VALUES ($1,$2) ON CONFLICT DO NOTHING', [txHash, battleId]);
  ...
  if (bRef) {
    ...
    console.log(`[ref] Chain for ${wallet}: ${JSON.stringify(chain)}`);
    if (chain.length > 0) {
      payReferrals(battleId, wallet, parseInt(bRef.stake)).catch(e =>
        console.error('[ref] payReferrals error:', e.message)
      );
    } else {
      console.log(`[ref] No referral chain for ${wallet} — skipping`);
    }
  }
  ...
  if (p?.tg_user_id) {
    const lang = await getUserLangById(p.tg_user_id);
    await sendTg(p.tg_user_id, MSG.stake_accepted[lang](battleId, paidCount, b.max_players));
  }
  ...
  if (paidCount >= b.max_players && b.status === 'open') {
    ...
    for (const part of parts.rows) {
      const lang_start = await getUserLangById(part.tg_user_id);
      await sendTg(part.tg_user_id, MSG.battle_started[lang_start](battleId, b.stake * b.max_players, prize));
    }
  }
}
...
async function checkPayments() {
  ...
  for (const tx of txs) {
    ...
    // Проверяем seen_txs с правильным battle_id
    if ((await q('SELECT 1 FROM seen_txs WHERE tx_hash=$1 AND battle_id=$2', [txHash, pend.battle_id])).rows.length) continue;
...
// Пересчёт пропущенных реф выплат за все прошлые битвы
// Исправить реферера для конкретного кошелька
app.post('/admin/fix-ref', requireAdmin, async (req, res) => {
  ...
  await q(
    'UPDATE user_settings SET referred_by=$1 WHERE wallet=$2',
    [referred_by, wallet]
  );
...
app.post('/admin/recalc-refs', requireAdmin, async (req, res) => {
  ...
  for (const row of battles.rows) {
    // Проверяем нет ли уже выплаты для этой битвы и кошелька
    const existing = await q(
      'SELECT id FROM referral_payouts WHERE battle_id=$1 AND from_wallet=$2',
      [row.battle_id, row.wallet]
    );
...
app.get('/admin/debug-tx/:hash', requireAdmin, async (req, res) => {
  const hash = req.params.hash.toUpperCase();
  const seen  = await q('SELECT * FROM seen_txs WHERE tx_hash=$1', [hash]);
  const pend  = await q('SELECT * FROM pending_payments WHERE verified=0');
  res.json({ seen_txs: seen.rows, pending: pend.rows });
});
...
app.get('/battles/:id/live', async (req, res) => {
  ...
  const participants = await Promise.all(parts.rows.map(async p => {
    ...
    const startAxm = start.total_axm || start.total_usd || 0;
    const startPrices = start.prices || {};

    // Считаем текущую стоимость стартового портфеля
    let curAxm = 0;
    for (const [sym, data] of Object.entries(start.tokens || {})) {
      curAxm += (data.amount || 0) * (prices[sym] || 0);
    }

    const growth = startAxm > 0 ? ((curAxm - startAxm) / startAxm) * 100 : 0;

    return {
      wallet:      p.wallet,
      tg_username: p.tg_username,
      growth_pct:  parseFloat(growth.toFixed(2)),
      start_axm:   parseFloat(startAxm.toFixed(4)),
      current_axm: parseFloat(curAxm.toFixed(4)),
    };
  }));
...
app.post('/battles/:id/join', requireWallet, async (req, res) => {
  ...
  const totalCnt = +(await q('SELECT COUNT(*) as c FROM participants WHERE battle_id=$1', [req.params.id])).rows[0].c;
  if (paidCnt >= b.max_players)  return res.status(400).json({ error: LANG === 'en' ? 'Battle is full' : 'Битва заполнена' });
  if (totalCnt >= b.max_players) return res.status(400).json({ error: LANG === 'en' ? 'Battle is full (all slots reserved)' : 'Все места заняты — ждут оплаты' });
...
app.post('/battles/:id/finalize', async (req, res) => {
  ...
  for (const loser of parts.filter(p => p.wallet !== winner)) {
    ...
    const lang_lose = await getUserLangById(loser.tg_user_id);
    const loserGrowth = parts.find(p=>p.wallet===loser.wallet)?.growth_pct || 0;
    await sendTg(loser.tg_user_id, MSG.loser[lang_lose](b.id, winner, loserGrowth));
  }
...
app.post('/admin/confirm-payment', requireAdmin, async (req, res) => {
  ...
  // Если участника нет — создаём его
  const p = (await q('SELECT * FROM participants WHERE battle_id=$1 AND wallet=$2', [battle_id, wallet])).rows[0];
  if (!p) {
    await q(
      'INSERT INTO participants (wallet, battle_id, tg_username, joined_at, paid) VALUES ($1,$2,$3,$4,0)',
      [wallet, battle_id, tg_username || wallet.slice(0,8), Math.floor(Date.now()/1000)]
    );
    // Создаём pending_payment
    await q(
      'INSERT INTO pending_payments (battle_id, wallet, amount, created_at) VALUES ($1,$2,$3,$4) ON CONFLICT DO NOTHING',
...
setInterval(async () => {
  ...
  for (const b of soonBattles.rows) {
    try {
      ...
      for (const p of parts.rows) {
        ...
        const lang_warn = await getUserLangById(p.tg_user_id);
        await sendTg(p.tg_user_id,
          MSG.warn_5min[lang_warn](b.id),
          { reply_markup: { inline_keyboard: [[{ text: MSG.watch_race[lang_warn], web_app: { url: `${CONFIG.FRONTEND_URL}?lang=${lang_warn}` } }]] } }
        );
      }
  ...
  // 2. Автоотмена: open битвы старше 2 часов — отменяем и возвращаем ставки
  const CANCEL_TIMEOUT_SEC = 2 * 60 * 60; // 2 часа
  const stale = await q(
  ...
  for (const b of stale.rows) {
    try {
      ...
      for (const p of paid.rows) {
        ...
        if (p.tg_user_id) {
          await sendTg(p.tg_user_id,
            `↩️ <b>Битва #${b.id} отменена</b>

Никто не присоединился за 2 часа.
Ставка <b>${b.stake} SLUG</b> возвращена на твой кошелёк автоматически.`
          );
        }
      }
      ...
      if (paid.rows.length === 0) {
        ...
        if (tgId) {
          await sendTg(tgId, `❌ <b>Битва #${b.id} отменена</b>
...
}, 30 * 1000); // каждые 30 секунд // каждые 30 секунд
