...
// Получаем цену AXM в USD через CoinGecko
async function getAxmPriceUsd() {
  try {
    const data = await (await fetch(
      'https://api.coingecko.com/api/v3/simple/price?ids=axiome&vs_currencies=usd',
      { signal: AbortSignal.timeout(4000) }
    )).json();
    return data?.axiome?.usd || 0.004;
  } catch { return 0.004; }
}

// Получаем цену токена из пула Axiome Swap
// Пул хранит резервы token1 и token2 (один из них AXM нативный)
...
async function getTokenPriceFromPool(poolAddr) {
  try {
    ...
    // Определяем какой токен AXM (native), какой — мем-токен
    const isToken1Native = d.token1_denom?.native === 'uaxm';
    const axmReserve   = isToken1Native ? parseFloat(d.token1_reserve) : parseFloat(d.token2_reserve);
    const tokenReserve = isToken1Native ? parseFloat(d.token2_reserve) : parseFloat(d.token1_reserve);

    if (!tokenReserve || tokenReserve === 0) return null;

    // Цена в AXM (оба резерва в uaxm/utoken с 6 decimals — сокращаются)
    return axmReserve / tokenReserve;
  } catch { return null; }
}

// Получаем цены всех токенов
async function getTokenPrices() {
  const axmUsd = await getAxmPriceUsd();
  const prices = { AXM: axmUsd };

  // Параллельно запрашиваем все пулы
  ...
  for (const result of poolResults) {
    if (result.status === 'fulfilled' && result.value.priceInAxm !== null) {
      const { symbol, priceInAxm } = result.value;
      prices[symbol] = priceInAxm * axmUsd;
    }
  }

  // SLUG fallback если пул недоступен
  if (!prices['SLUG']) prices['SLUG'] = 0.008;
