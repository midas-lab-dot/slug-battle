...
// ── Автовыплата через Amino REST ────────────────────────────
// Используем Secp256k1HdWallet из @cosmjs/amino
// Подписываем через Amino JSON и отправляем через REST /txs

let _aminoWallet = null;

async function getAminoWallet() {
  if (_aminoWallet) return _aminoWallet;

  const privkeyHex = process.env.PROJECT_WALLET_PRIVKEY;
  const mnemonic   = process.env.PROJECT_WALLET_MNEMONIC;

  if (privkeyHex) {
    // Secp256k1Wallet поддерживает signAmino
    const { Secp256k1Wallet } = require('@cosmjs/amino');
    const privkeyBytes = Uint8Array.from(Buffer.from(privkeyHex, 'hex'));
    _aminoWallet = await Secp256k1Wallet.fromKey(privkeyBytes, 'axm');
  } else if (mnemonic) {
    const { Secp256k1HdWallet } = require('@cosmjs/amino');
    _aminoWallet = await Secp256k1HdWallet.fromMnemonic(mnemonic, { prefix: 'axm' });
  } else {
    throw new Error('PROJECT_WALLET_PRIVKEY or PROJECT_WALLET_MNEMONIC required');
  }

  const accts = await _aminoWallet.getAccounts();
  console.log(`[payout] Wallet loaded: ${accts[0].address}`);

  if (accts[0].address !== CONFIG.PROJECT_WALLET) {
    console.error(`[payout] ⚠️ Address mismatch: got ${accts[0].address}, expected ${CONFIG.PROJECT_WALLET}`);
  }
...
async function autoPayout(battleId, winner, prizeSlug) {
  try {
    const wallet   = await getAminoWallet();
    const accounts = await wallet.getAccounts();
    const sender   = accounts[0].address;
    const prizeRaw = String(Math.round(prizeSlug * 1e6));

    if (sender !== CONFIG.PROJECT_WALLET) {
      throw new Error(`Wallet mismatch: got ${sender}, expected ${CONFIG.PROJECT_WALLET}`);
    }

    // Получаем account_number и sequence через REST
    const accRes  = await fetch(`${CONFIG.AXIOME_REST}/cosmos/auth/v1beta1/accounts/${sender}`, { signal: AbortSignal.timeout(8000) });
    const accData = await accRes.json();
    const acc     = accData?.account;
    if (!acc) throw new Error(`Account not found: ${JSON.stringify(accData)}`);

    const accountNumber = parseInt(acc.account_number);
    const sequence      = parseInt(acc.sequence);
    console.log(`[payout] account_number=${accountNumber} sequence=${sequence}`);
    ...
    const signDoc = {
      ...
      fee: {
        amount: [{ denom: 'uaxm', amount: '600000' }],
        gas:    '200000',
      },
      msgs: [{
        ...
        value: {
          sender,
          contract: CONFIG.SLUG_CONTRACT,
          msg:      btoa(JSON.stringify({ transfer: { recipient: winner, amount: prizeRaw } })),
          funds:    [],
        },
      }],
      memo: `WIN:${battleId}`,
    ...
    // Подписываем через Amino
    const signed = await wallet.signAmino(sender, signDoc);

    // Формируем StdTx и отправляем через /cosmos/tx/v1beta1/txs
    // Используем AminoTypes для конвертации в Proto
    const { makeStdTx } = require('@cosmjs/amino');
    const stdTx = makeStdTx(signDoc, signed.signature);

    // Отправляем через broadcast_tx
    const broadcastRes = await fetch(`${CONFIG.AXIOME_REST}/cosmos/tx/v1beta1/txs`, {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({
        tx_bytes: Buffer.from(JSON.stringify(stdTx)).toString('base64'),
    ...
    console.log('[payout] broadcast result:', JSON.stringify(result).slice(0, 500));

    const txRes = result.tx_response || result;
    if (txRes.code && txRes.code !== 0) throw new Error(txRes.raw_log || `code=${txRes.code}`);

    const txHash = txRes.txhash;
    console.log(`[payout] ✅ Prize sent: battle=${battleId} winner=${winner} prize=${prizeSlug} SLUG tx=${txHash}`);
...
async function getReferralChain(wallet) {
  ...
  for (let level = 1; level <= 3; level++) {
    const r = await q(
      'SELECT referred_by FROM user_settings WHERE wallet=$1 AND referred_by IS NOT NULL',
      [current]
    );
...
app.post('/register', async (req, res) => {
  const { wallet, tg_user_id } = req.body;
  if (!wallet?.startsWith('axm1')) return res.status(400).json({ error: 'Invalid wallet' });

  await q(`INSERT INTO user_settings (tg_user_id, wallet)
    VALUES ($1, $2)
    ON CONFLICT (tg_user_id) DO UPDATE SET wallet=$2, updated_at=EXTRACT(EPOCH FROM NOW())`,
    [String(tg_user_id || ''), wallet]
  );
...
app.get('/referral/:wallet', async (req, res) => {
  ...
  res.json({
    wallet,
    ref_link:      refLink,
    total_refs:    parseInt(refs.rows[0].count) || 0,
