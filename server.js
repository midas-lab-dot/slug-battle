...
const CONFIG = {
  ...
  SWAP_POOLS: [
    ...
    { symbol: 'HUNT',   pool: 'axm1zenms99e9f4rq9wjyms69s7gg0k3th7rdnlu26x3aatac3cqxc0sanl5dh', decimals: 6 },
  ...
  if (result.ok) {
    ...
    if (p?.tg_user_id) {
      await sendTg(p.tg_user_id,
        MSG.winner_ok[(await getUserLangByWallet(winner)) === 'ru' ? 'ru' : 'en'](battle.id, prize, result.txHash)
      );
    }
    // Уведомляем себя
    ...
    if (p?.tg_user_id) {
      await sendTg(p.tg_user_id,
        MSG.winner_pending[(await getUserLangByWallet(winner)) === 'ru' ? 'ru' : 'en'](battle.id, prize)
      );
    }
  }
}
...
async function notifyWinnerAuto(battle, winner, prize) {
  ...
  await sendTg(p.tg_user_id, MSG.winner_ok[_langW === 'ru' ? 'ru' : 'en'](battle.id, prize, ''));
}
...
async function confirmPayment(battleId, wallet, txHash, paidAmount) {
  ...
  if (battleCheck.status === 'cancelled' || battleCheck.status === 'ended') {
    console.log(`[watcher] Skip: battle ${battleId} is ${battleCheck.status} — no refund (manual if needed)`);
    return;
  }
  ...
  if (currentPaid >= battleCheck.max_players) {
    console.log(`[watcher] Skip: battle ${battleId} already full (${currentPaid}/${battleCheck.max_players}) — no auto-refund`);
    return;
  }
  ...
  if (p?.tg_user_id) {
    ...
    await sendTg(p.tg_user_id, MSG.stake_accepted[_lang1 === 'ru' ? 'ru' : 'en'](battleId, paidCount, b.max_players));
  }
  ...
  if (paidCount >= b.max_players) {
    ...
    if (updateResult.rows.length > 0) {
      ...
      for (const part of parts.rows) {
        ...
        await sendTg(part.tg_user_id, MSG.battle_started[_lang2 === 'ru' ? 'ru' : 'en'](battleId, parseInt(b.stake) * b.max_players, prize));
      }
    }
  }
}
...
async function checkPayments() {
  ...
  for (const tx of txs) {
    ...
    // Ищем pending платёж по memo или по кошельку
    let pend = null;
    if (memoData?.battleId && memoData?.wallet) {
      pend = active.find(p => p.battle_id === memoData.battleId && p.wallet === memoData.wallet);
    }
    if (!pend) {
...
app.post('/battles/:id/finalize', async (req, res) => {
  ...
  for (const loser of parts.filter(p => p.wallet !== winner)) {
    ...
    await sendTg(loser.tg_user_id, MSG.loser[_langL === 'ru' ? 'ru' : 'en'](b.id, winner, _growth));
  }
...
app.post('/admin/force-start', requireAdmin, async (req, res) => {
  ...
  for (const p of parts.rows) {
    ...
    await sendTg(p.tg_user_id, MSG.battle_started[lang === 'ru' ? 'ru' : 'en'](battle_id, parseInt(b.stake) * paidCount, prize));
  }
...
// ── Выплатить все ended битвы без выплат ────────────────────
app.post('/admin/payout-all-pending', requireAdmin, async (req, res) => {
  try {
    // Находим ended битвы где победитель есть но нет записи о выплате в seen_txs с memo WIN:id
    const battles = (await q(
      `SELECT b.* FROM battles b
       WHERE b.status = 'ended'
       AND b.winner IS NOT NULL
       AND NOT EXISTS (
         SELECT 1 FROM seen_txs st WHERE st.tx_hash LIKE 'WIN_PAID:' || b.id || '%'
       )
       ORDER BY b.created_at DESC
       LIMIT 50`
    )).rows;

    if (!battles.length) return res.json({ ok: true, message: 'No pending payouts', count: 0 });

    console.log(`[admin] Processing ${battles.length} pending payouts`);
    res.json({ ok: true, total: battles.length, message: 'Processing in background, check logs' });

    // Выплачиваем в фоне
    (async () => {
      for (const b of battles) {
        const prize = Math.round(parseInt(b.stake) * b.max_players * (1 - CONFIG.FEE_PERCENT / 100));
        try {
          const result = await autoPayout(b.id, b.winner, prize);
          if (result.ok) {
            // Отмечаем как выплаченное
            await q('INSERT INTO seen_txs (tx_hash) VALUES ($1) ON CONFLICT DO NOTHING',
              [`WIN_PAID:${b.id}:${result.txHash}`]);
            console.log(`[admin] ✅ Paid ${b.id} → ${b.winner} ${prize} SLUG tx=${result.txHash}`);
          } else {
            console.error(`[admin] ❌ Failed ${b.id}: ${result.error}`);
          }
        } catch (e) {
          console.error(`[admin] ❌ Error ${b.id}: ${e.message}`);
        }
      }
      console.log('[admin] payout-all-pending complete');
    })();
...
const MSG = {
  ...
  loser: {
    ru: (id, winner, growth) => `😔 <b>Битва #${id} завершена</b>\n\nПобедитель: @${winner}\nТвой результат: ${growth.toFixed(2)}%`,
    en: (id, winner, growth) => `😔 <b>Battle #${id} ended</b>\n\nWinner: @${winner}\nYour result: ${growth.toFixed(2)}%`,
  },
  warn_5min: {
...
app.post(`/webhook/${CONFIG.BOT_TOKEN}`, async (req, res) => {
  ...
  if (callback) {
    ...
    if (data === 'notify_on' || data === 'notify_off') {
      ...
      await q(`ALTER TABLE user_settings ADD COLUMN IF NOT EXISTS notify_new_battle INTEGER DEFAULT 0`).catch(() => {});
      ...
      const txt = lang === 'ru'
        ? (val ? '✅ Уведомления включены! Теперь ты будешь получать сообщение каждый раз когда создаётся новая битва.' : '❌ Уведомления выключены.')
        : (val ? '✅ Notifications enabled! You will receive a message every time a new battle is created.' : '❌ Notifications disabled.');

      // Send with toggle button
      await sendTg(chatId, txt, {
        reply_markup: {
          inline_keyboard: [[{
            text: lang === 'ru' ? (val ? '🔕 Выключить уведомления' : '🔔 Включить уведомления') : (val ? '🔕 Disable notifications' : '🔔 Enable notifications'),
            callback_data: val ? 'notify_off' : 'notify_on'
          }]]
        }
      });
      return;
    ...
    if (data === 'how_to_play_ru' || data === 'how_to_play_en') {
      const lang = data.includes('_ru') ? 'ru' : 'en';
      const t = I18N[lang];
      await fetch(`https://api.telegram.org/bot${CONFIG.BOT_TOKEN}/answerCallbackQuery`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ callback_query_id: callback.id }),
      ...
      await sendTg(chatId, t.welcome, {
        reply_markup: {
          inline_keyboard: [
            ...
            [{ text: lang==='ru' ? '🔔 Уведомления о битвах' : '🔔 Battle notifications', callback_data: 'notify_on' }],
          ]
        }
      });
    }
  ...
  for (const b of soonBattles.rows) {
    try {
      ...
      for (const p of parts.rows) {
        ...
        await sendTg(p.tg_user_id,
          MSG.warn_5min[_l5 === 'ru' ? 'ru' : 'en'](b.id),
          { reply_markup: { inline_keyboard: [[{ text: MSG.watch_race[_l5 === 'ru' ? 'ru' : 'en'], web_app: { url: `${CONFIG.FRONTEND_URL}?lang=${_l5}` } }]] } }
        );
      }
  ...
  for (const b of stale.rows) {
    try {
      ...
      for (const p of paid.rows) {
        const result = await autoPayout(`REFUND:${b.id}:${p.wallet.slice(-6)}`, p.wallet, parseInt(b.stake));
        if (result.ok) {
        ...
        if (p.tg_user_id) {
          ...
          await sendTg(p.tg_user_id, MSG.refund[_lR === 'ru' ? 'ru' : 'en'](b.id, b.stake));
        }
      }
      ...
      if (paid.rows.length === 0) {
        ...
        if (tgId) {
          ...
          await sendTg(tgId, MSG.cancel_empty[_lC === 'ru' ? 'ru' : 'en'](b.id));
        }
      }
