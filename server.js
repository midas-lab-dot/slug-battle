  ...
  if (result.ok) {
    ...
    if (p?.tg_user_id) {
      await sendTg(p.tg_user_id,
        MSG.winner_ok[await getUserLangByWallet(winner)](battle.id, prize, result.txHash)
      );
    }
    // Уведомляем себя
    ...
    if (p?.tg_user_id) {
      await sendTg(p.tg_user_id,
        MSG.winner_pending[await getUserLangByWallet(winner)](battle.id, prize)
      );
    }
  }
}
...
async function notifyWinnerAuto(battle, winner, prize) {
  ...
  await sendTg(p.tg_user_id, MSG.winner_ok[_langW](battle.id, prize, ''));
}
...
async function confirmPayment(battleId, wallet, txHash, paidAmount) {
  ...
  if (!battleCheck || battleCheck.status !== 'open') {
    console.log(`[watcher] Skip: battle ${battleId} not open (status=${battleCheck?.status})`);
    // Возвращаем ставку если битва уже стартовала или отменена
    if (battleCheck?.status === 'live' || battleCheck?.status === 'cancelled') {
      const refundResult = await autoPayout(battleId + '_refund_' + wallet.slice(-4), wallet, paidAmount / 1e6);
      console.log(`[watcher] Refund sent to ${wallet}: ${JSON.stringify(refundResult)}`);
    }
    return;
  }
  ...
  if (currentPaid >= battleCheck.max_players) {
    console.log(`[watcher] Skip: battle ${battleId} already full (${currentPaid}/${battleCheck.max_players})`);
    // Возвращаем лишнюю ставку
    const refundResult = await autoPayout('REFUND_FULL_' + battleId, wallet, paidAmount / 1e6);
    console.log(`[watcher] Refund sent to ${wallet}: ${JSON.stringify(refundResult)}`);
    return;
  }
  ...
  if (p?.tg_user_id) {
    ...
    await sendTg(p.tg_user_id, MSG.stake_accepted[_lang1](battleId, paidCount, b.max_players));
  }
  ...
  if (paidCount >= b.max_players && b.status === 'open') {
    ...
    for (const part of parts.rows) {
      ...
      await sendTg(part.tg_user_id, MSG.battle_started[_lang2](battleId, b.stake * b.max_players, prize));
    }
  }
}
...
app.post('/battles/:id/join', requireWallet, async (req, res) => {
  ...
  // Атомарная проверка — с блокировкой строки чтобы исключить race condition
...
app.post('/battles/:id/finalize', async (req, res) => {
  ...
  for (const loser of parts.filter(p => p.wallet !== winner)) {
    ...
    await sendTg(loser.tg_user_id, MSG.loser[_langL](b.id, winner, _growth));
  }
...
app.post(`/webhook/${CONFIG.BOT_TOKEN}`, async (req, res) => {
  ...
  if (msg.text === '/notify') {
    const lang = await getUserLang(userId) || 'ru';
    const cur = (await q('SELECT notify_new_battle FROM user_settings WHERE tg_user_id=$1', [String(userId)])).rows[0];
  ...
  for (const b of soonBattles.rows) {
    try {
      ...
      for (const p of parts.rows) {
        ...
        await sendTg(p.tg_user_id,
          MSG.warn_5min[_l5](b.id),
          { reply_markup: { inline_keyboard: [[{ text: MSG.watch_race[_l5], web_app: { url: `${CONFIG.FRONTEND_URL}?lang=${_l5}` } }]] } }
        );
      }
    ...
    try {
      ...
      for (const p of paid.rows) {
        ...
        if (p.tg_user_id) {
          ...
          await sendTg(p.tg_user_id, MSG.refund[_lR](b.id, b.stake));
        }
      }
      ...
      if (paid.rows.length === 0) {
        ...
        if (tgId) {
          ...
          await sendTg(tgId, MSG.cancel_empty[_lC](b.id));
        }
      }
