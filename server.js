...
// MSG поддерживает только ru и en — остальные языки → en
  ...
  if (result.ok) {
    ...
    if (p?.tg_user_id) {
      await sendTg(p.tg_user_id,
        MSG.winner_ok[msgLang(await getUserLangByWallet(winner))](battle.id, prize, result.txHash)
      );
    }
    // Уведомляем себя
    ...
    if (p?.tg_user_id) {
      await sendTg(p.tg_user_id,
        MSG.winner_pending[msgLang(await getUserLangByWallet(winner))](battle.id, prize)
      );
    }
  }
}
...
async function notifyWinnerAuto(battle, winner, prize) {
  ...
  await sendTg(p.tg_user_id, MSG.winner_ok[msgLang(_langW)](battle.id, prize, ''));
}
...
async function confirmPayment(battleId, wallet, txHash, paidAmount) {
  ...
  if (p?.tg_user_id) {
    ...
    await sendTg(p.tg_user_id, MSG.stake_accepted[msgLang(_lang1)](battleId, paidCount, b.max_players));
  }
  ...
  if (paidCount >= b.max_players) {
    ...
    if (updateResult.rows.length > 0) {
      ...
      for (const part of parts.rows) {
        ...
        await sendTg(part.tg_user_id, MSG.battle_started[msgLang(_lang2)](battleId, b.stake * b.max_players, prize));
      }
    }
  }
}
...
app.post('/battles/:id/finalize', async (req, res) => {
  ...
  for (const loser of parts.filter(p => p.wallet !== winner)) {
    ...
    await sendTg(loser.tg_user_id, MSG.loser[msgLang(_langL)](b.id, winner, _growth));
  }
...
app.post('/admin/force-start', requireAdmin, async (req, res) => {
  ...
  for (const p of parts.rows) {
    ...
    await sendTg(p.tg_user_id, MSG.battle_started[msgLang(lang)](battle_id, parseInt(b.stake) * paidCount, prize));
  }
  ...
  for (const b of soonBattles.rows) {
    try {
      ...
      for (const p of parts.rows) {
        ...
        await sendTg(p.tg_user_id,
          MSG.warn_5min[msgLang(_l5)](b.id),
    ...
    try {
      ...
      for (const p of paid.rows) {
        ...
        if (p.tg_user_id) {
          ...
          await sendTg(p.tg_user_id, MSG.refund[msgLang(_lR)](b.id, b.stake));
        }
      }
      ...
      if (paid.rows.length === 0) {
        ...
        if (tgId) {
          ...
          await sendTg(tgId, MSG.cancel_empty[msgLang(_lC)](b.id));
        }
      }
