...
async function getSigningClient() {
  ...
  for (const rpc of endpoints) {
    try {
      ...
      console.log(`[cosmjs] Connected to RPC: ${rpc}`);
      return _signingClient;
    } catch (e) {
      console.warn(`[cosmjs] Failed to connect to ${rpc}: ${e.message}`);
      lastError = e;
    }
  }
  throw lastError;
...
// Сбрасываем клиент при ошибке чтобы переподключиться
...
app.get('/referral/:wallet', async (req, res) => {
  ...
  const refLink = `https://t.me/${process.env.BOT_USERNAME || 'CryptoBattleSLUGBot'}?start=ref_${wallet}`;
