...
app.post(`/webhook/${CONFIG.BOT_TOKEN}`, async (req, res) => {
  ...
  if (callback) {
    ...
    if (data === 'lang_ru' || data === 'lang_en' || data === 'lang_de' || data === 'lang_bg') {
    ...
    if (data === 'how_to_play_ru' || data === 'how_to_play_en' || data === 'how_to_play_de' || data === 'how_to_play_bg') {
      const lang = data.includes('_ru') ? 'ru' : data.includes('_de') ? 'de' : data.includes('_bg') ? 'bg' : 'en';
      const t = I18N[lang];
      await fetch(`https://api.telegram.org/bot${CONFIG.BOT_TOKEN}/answerCallbackQuery`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ callback_query_id: callback.id }),
