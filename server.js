...
async function confirmPayment(battleId, wallet, txHash, paidAmount) {
  ...
  if (paidCount >= b.max_players && b.status === 'open') {
    const durationMs = parseInt(b.duration_ms) || 600000; // fallback 10 минут
...
app.post('/battles/:id/join', requireWallet, async (req, res) => {
  ...
  // Антиабуз: лимит 10 битв между одной парой кошельков за турнир
  const activeTournament = (await q("SELECT id, starts_at FROM tournament WHERE status='active' ORDER BY id DESC LIMIT 1")).rows[0];
  if (activeTournament) {
    const pairCount = +(await q(
      `SELECT COUNT(*) as c FROM battles b
       JOIN participants p1 ON p1.battle_id = b.id AND p1.wallet = $1 AND p1.paid = 1
       JOIN participants p2 ON p2.battle_id = b.id AND p2.wallet = $2 AND p2.paid = 1
       WHERE b.created_at >= $3`,
      [req.wallet, b.creator, activeTournament.starts_at]
    )).rows[0].c;
    if (pairCount >= 10) {
      return res.status(400).json({
        error: LANG === 'en'
          ? 'Anti-abuse: max 10 battles between same pair per tournament'
          : 'Антиабуз: максимум 10 битв между одной парой кошельков за турнир'
      });
    }
  }
...
app.post('/admin/force-start', requireAdmin, async (req, res) => {
  ...
  for (const p of parts.rows) {
    ...
    await sendTg(p.tg_user_id, MSG.battle_started[lang](battle_id, parseInt(b.stake) * paidCount, prize));
  }
