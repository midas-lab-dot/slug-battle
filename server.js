...
async function getAminoWallet() {
  ...
  if (privkeyHex) {
    // Используем приватный ключ напрямую
    const privkeyBytes = Uint8Array.from(Buffer.from(privkeyHex, 'hex'));
    _aminoWallet = await DirectSecp256k1Wallet.fromKey(privkeyBytes, 'axm');
  } else if (mnemonic) {
    _aminoWallet = await DirectSecp256k1HdWallet.fromMnemonic(mnemonic, { prefix: 'axm' });
  } else {
  ...
  if (accts[0].address !== CONFIG.PROJECT_WALLET) {
    console.error(`[payout] ⚠️ Address mismatch! Key gives ${accts[0].address}, expected ${CONFIG.PROJECT_WALLET}`);
  }
...
async function autoPayout(battleId, winner, prizeSlug) {
  try {
    ...
    if (result.code && result.code !== 0) throw new Error(result.raw_log || `code=${result.code}`);
