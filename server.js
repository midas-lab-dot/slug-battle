...
async function getSigningClient() {
  ...
  _signerWallet = await DirectSecp256k1HdWallet.fromMnemonic(mnemonic, { prefix: 'axm' });

  // Пробуем разные RPC endpoints
  const endpoints = [
    'http://api-docs.axiomeinfo.org:26657',
    'https://rpc.axiome.pro',
    'http://rpc.axiome.pro:26657',
  ];

  for (const rpc of endpoints) {
    try {
      _signingClient = await SigningCosmWasmClient.connectWithSigner(rpc, _signerWallet, {
        gasPrice: GasPrice.fromString('0.025uaxm'),
      });
      const chainId = await _signingClient.getChainId();
      console.log(`[cosmjs] Connected to ${rpc}, chainId=${chainId}`);
      return _signingClient;
    } catch (e) {
      console.warn(`[cosmjs] ${rpc} failed: ${e.message}`);
      _signingClient = null;
    }
  }
...
async function autoPayout(battleId, winner, prizeSlug) {
  try {
    ...
    // Проверяем что клиент видит наш аккаунт
    const chainId = await client.getChainId();
    console.log(`[payout] ChainId: ${chainId}, sending ${prizeSlug} SLUG to ${winner}`);
