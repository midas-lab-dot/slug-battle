...
app.post('/admin/payout-all-pending', requireAdmin, async (req, res) => {
  try {
    const battles = (await q(
      "SELECT * FROM battles WHERE status='ended' AND winner IS NOT NULL AND payout_tx IS NULL"
    )).rows;
    ...
    for (const b of battles) {
      ...
      try {
        ...
        if (result.ok) {
          await q('UPDATE battles SET payout_tx=$1 WHERE id=$2', [result.txHash, b.id]);
          results.push({ id: b.id, ok: true, winner: b.winner, prize, tx: result.txHash });
          console.log(`[admin] Paid battle ${b.id} → ${b.winner} ${prize} SLUG tx=${result.txHash}`);
        } else {
          results.push({ id: b.id, ok: false, winner: b.winner, prize, error: result.error });
          console.error(`[admin] Failed battle ${b.id}: ${result.error}`);
        }
      } catch (e) {
        results.push({ id: b.id, ok: false, error: e.message });
      }
    ...
    res.json({ ok: true, total: battles.length, paid: ok, failed: fail, results });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
