...
async function autoPayout(battleId, winner, prizeSlug) {
  try {
    ...
    const sequence      = String(acc.sequence);
    ...
    const signDoc = {
      ...
      msgs: [{
        ...
        value: {
          ...
          msg:      { transfer: { recipient: winner, amount: prizeRaw } },
          funds:    [],
        },
      }],
      memo: `WIN:${battleId}`,
    ...
    // Отправляем через legacy /txs endpoint
    const broadcastRes = await fetch(`${CONFIG.AXIOME_REST}/txs`, {
      method:  'POST',
    ...
    if (result.code && result.code !== 0) throw new Error(result.raw_log || result.message || `code=${result.code}`);
